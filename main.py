...
class BlogSecurity:
    """ This class contains all methods to check the security if each link and 
        cookie of the website. It contains methods to validate the username,
        the password and the email entered while a registration.
...
class User(ndb.Model):
    """ This class model represent article's author: each user has a unique 
...
class Post(ndb.Model):
    ...
    each post has a title, a content, creation date, a user id of 
    its author and its number of like
    """
...
class Comments(ndb.Model):
    """ This class model represent comments on post: a comment is describe by 
    a content, the post_id and the user id who comment the post"""
...
class Handler(webapp2.RequestHandler):
    ...
    def set_user_cookie(self, value):
        self.response.headers.add_header(
            'Set-Cookie', 'user=%s' %
            BlogSecurity.make_secure_val(value))
...
class DeletePost(Handler):
    ...
    def get(self, post_id):
        ...
        post = key.get()
        if post.author != self.current_user().key:
            return self.redirect('/blog/%s' % key.id())
...
class EditComment(Handler):
    ...
    def post(self, comment_id):
        ...
        if comment:
            ...
            post_comment.put()
        return self.redirect('/blog/%s' % post_comment.post_id.id())
...
class LikePost(Handler):
    ...
    def get(self, post_id):
        ...
        if post and (post.author != current_user.key):
            if key not in current_user.post_likes:
...
class Login(Handler):
    ...
    def post(self):
        user = User.query(
            User.username == self.request.get('username')).fetch()

        if user and BlogSecurity.check_secure_pass(
                self.request.get('password'), user[0].passwd):
